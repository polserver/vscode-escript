 ...
 */
export function deepEquals(value1: any, value2: any, stack = new Array<any>()) {
    ...
    if (type1 === 'bigint' || type1 === 'boolean' || type1 === 'string' || type1 === 'symbol') {
        return false;
    }
    ...
    if (Array.isArray(value1)) {
        ...
        for (let i = 0; i < length; i++) {
            if (!deepEquals(value1[i], value2[i], stack)) {
                return false;
            }
        }
        return true;
    ...
    for (let i = 0; i < numKeys; i++) {
        if (!deepEquals(value1[keys1[i]], value2[keys1[i]], stack)) {
            return false;
        }
    }
